...
class PK6 {
  constructor(binary, posBox) {
    ...
    for (let i = 0; i < 0x104; i++) this._bin[i] = binary[i];
  ...
  SaveToFile(path) {
    ...
    fs.writeFileSync(
      path,
      String.fromCharCode.apply(null, this._bin),
      'binary'
    );
  }
  ...
  _getIVs() {
    let memory = Memory.RW.getValueAt(
      this._offset.INDIVIDUAL_VALUES,
      this._bin
    );
  ...
  GetNationalID() {
    let memory = Memory.RW.getValueAt(
      this._offset.NATIONAL_POKEDEX_ID,
      this._bin
    );
  ...
  GetPID() {
    let memory = Memory.RW.getValueAt(
      this._offset.PERSONALITY_VALUE,
      this._bin
    );
    let u32a = new Uint32Array(memory.buffer);
    return u32a[0];
  ...
  SetPID(pid) {
    ...
    let memory = new Uint8Array(buffer.buffer);
    Memory.RW.setValueAt(this._offset.PERSONALITY_VALUE, memory, this._bin);
  }
  SetPIDHex(pidHex) {
  ...
  GetEVs() {
    ...
    let memoryDEFENSE = Memory.RW.getValueAt(
      this._offset.EV_DEFENSE,
      this._bin
    );
    let memorySPECIALATTACK = Memory.RW.getValueAt(
      this._offset.EV_SPECIALATTACK,
      this._bin
    );
    let memorySPECIALDEFENSE = Memory.RW.getValueAt(
      this._offset.EV_SPECIALDEFENSE,
      this._bin
    );
  ...
  SetEVs(evs) {
    ...
    tmpEVs.specialattack = new Uint8Array([
      evs.specialattack || current.specialattack
    ]);
    tmpEVs.specialdefense = new Uint8Array([
      evs.specialdefense || current.specialdefense
    ]);
    ...
    Memory.RW.setValueAt(
      this._offset.EV_SPECIALATTACK,
      tmpEVs.specialattack,
      this._bin
    );
    Memory.RW.setValueAt(
      this._offset.EV_SPECIALDEFENSE,
      tmpEVs.specialdefense,
      this._bin
    );
  }
  ...
  GetMoveRelearn(pos) {
    let memory = Memory.RW.getValueAt(
      this._offset['MOVE_RELEARN_' + pos],
      this._bin
    );
    let u16a = new Uint16Array(memory.buffer);
    return u16a[0];
